  ...
  .controller('EventsCtrl', function($scope,$timeout, socket, CalendarEvent, Auth) {
    ...
    $scope.$watch('newEvent.edate', function handleDateChange(newValue, oldValue) {
      if (!newValue){
        ...
        //$scope.newEvent.edate = moment.utc($scope.newEvent.start).format("MMM, D dddd");
      }
        
    });
    $scope.$watch(
      function () { return $("#edate")[0].value; },
      function (newValue, oldValue) {
        
        if (newValue !== oldValue && newValue !== "") {
          $scope.newEvent.edate = moment.utc($("#edate")[0].value).format("MMM, D dddd");
        }
      }
    );
    $scope.isEdit=false;
